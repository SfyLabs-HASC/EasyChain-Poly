...
const RefreshIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path fillRule="evenodd" d="M8 3a5 5 0 1 0 4.546 2.914.5.5 0 0 1 .908-.417A6 6 0 1 1 8 2z" /><path d="M8 4.466V.534a.25.25 0 0 1 .41-.192l2.36 1.966c.12.1.12.284 0 .384L8.41 4.658A.25.25 0 0 1 8 4.466" /></svg> );
...
const BatchRow = ({ batch, localId }: { batch: BatchData; localId: number; }) => {
    ...
    const { incrementRpcCount } = useRpcCounter();
    const { data: stepCount } = useReadContract({
        ...
        queryOptions: { onSuccess: () => { console.log(`RPC: getBatchStepCount for ${batch.batchId}`); incrementRpcCount(); }}
    });
    const formatDate = (dateStr: string | undefined) => !dateStr || dateStr.split("-").length !== 3 ? "/" : dateStr.split("-").reverse().join("/");
    return ( <> <tr className="desktop-row"> <td>{localId}</td> <td><span className="clickable-name" onClick={() => setShowDescription(true)}>{batch.name || "/"}</span></td> <td>{formatDate(batch.date)}</td> <td>{batch.location || "/"}</td> <td>{stepCount !== undefined ? stepCount.toString() : "/"}</td> <td>{batch.isClosed ? <span className="status-closed">✅ Chiuso</span> : <span className="status-open">⏳ Aperto</span>}</td> <td><Link to={`/gestisci/${batch.batchId}`} className="web3-button">Gestisci</Link></td> </tr> <tr className="mobile-card"> <td> <div className="card-header"> <strong className="clickable-name" onClick={() => setShowDescription(true)}>{batch.name || "N/A"}</strong> <span className={`status ${batch.isClosed ? "status-closed" : "status-open"}`}>{batch.isClosed ? "✅ Chiuso" : "⏳ Aperto"}</span> </div> <div className="card-body"> <p><strong>ID:</strong> {localId}</p> <p><strong>Data:</strong> {formatDate(batch.date)}</p> <p><strong>Luogo:</strong> {batch.location || "/"}</p> <p><strong>N° Passaggi:</strong> {stepCount !== undefined ? stepCount.toString() : "/"}</p> </div> <div className="card-footer"><Link to={`/gestisci/${batch.batchId}`} className="web3-button">Gestisci</Link></div> </td> </tr> {showDescription && ( <div className="modal-overlay" onClick={() => setShowDescription(false)}> <div className="modal-content description-modal" onClick={(e) => e.stopPropagation()}> <div className="modal-header"><h2>Descrizione Iscrizione / Lotto</h2></div> <div className="modal-body"><p>{batch.description || "Nessuna descrizione fornita."}</p></div> <div className="modal-footer"><button onClick={() => setShowDescription(false)} className="web3-button">Chiudi</button></div> </div> </div> )} </> );
};
const BatchTable = ({ batches, nameFilter, setNameFilter, locationFilter, setLocationFilter, statusFilter, setStatusFilter }: any) => {
    ...
    return ( <div className="table-container"> <table className="company-table"> <thead> <tr className="desktop-row"> <th>ID</th><th>Nome</th><th>Data</th><th>Luogo</th><th>N° Passaggi</th><th>Stato</th><th>Azione</th> </tr> <tr className="filter-row"> <th></th> <th><input type="text" placeholder="Filtra per nome..." className="filter-input" value={nameFilter} onChange={(e) => setNameFilter(e.target.value)} /></th> <th></th> <th><input type="text" placeholder="Filtra per luogo..." className="filter-input" value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)} /></th> <th></th> <th> <select className="filter-input" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}> <option value="all">Tutti</option><option value="open">Aperto</option><option value="closed">Chiuso</option> </select> </th> <th></th> </tr> </thead> <tbody> {visibleBatches.length > 0 ? ( visibleBatches.map((batch: BatchData, index: number) => ( <BatchRow key={batch.id} batch={batch} localId={startIndex + index + 1} /> )) ) : ( <tr><td colSpan={7} style={{ textAlign: "center", padding: "2rem" }}>Nessuna iscrizione trovata.</td></tr> )} </tbody> </table> <div className="pagination-controls"> {itemsToShow < itemsOnCurrentPage.length && (<button onClick={handleLoadMore} className="link-button">Vedi altri 10...</button>)} <div className="page-selector"> {totalPages > 1 && ( <> <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1}>&lt;</button> <span> Pagina {currentPage} di {totalPages} </span> <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages}>&gt;</button> </> )} </div> </div> </div> );
};
const DashboardHeader = ({ contributorInfo, onNewInscriptionClick, onRefreshClick, isRefreshDisabled, refreshTooltip }: any) => {
    const companyName = contributorInfo[0] || "Azienda";
    const credits = contributorInfo[1].toString();
    return ( <div className="dashboard-header-card"> <div className="dashboard-header-info"> <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}> <h2 className="company-name-header">{companyName}</h2> <button onClick={onRefreshClick} disabled={isRefreshDisabled} title={refreshTooltip} className="web3-button" style={{ padding: "0.5rem", backgroundColor: isRefreshDisabled ? "#495057" : "#6c757d", cursor: isRefreshDisabled ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", width: "40px", height: "40px" }}><RefreshIcon /></button> </div> <div className="company-status-container"> <div className="status-item"><span>Crediti Rimanenti: <strong>{credits}</strong></span></div> <div className="status-item"><span>Stato: <strong>ATTIVO</strong></span><span className="status-icon">✅</span></div> </div> </div> <div className="header-actions"><button className="web3-button large" onClick={onNewInscriptionClick}>Nuova Iscrizione</button></div> </div> );
};

export default function AziendaPage() {
    const account = useActiveAccount();
    const activeAddress = account?.address;
    const { incrementRpcCount, resetRpcCount } = useRpcCounter();
    ...
    const { data: contributorData, isLoading: isStatusLoading, refetch: refetchContributorInfo, isError } = useReadContract({
        ...
        params: activeAddress ? [activeAddress] : undefined,
        queryOptions: { enabled: !!activeAddress, onSuccess: () => { console.log("RPC: getContributorInfo"); incrementRpcCount(); } },
    });
    ...
    const prevAccountRef = useRef(activeAddress);

    const fetchAllBatchesOnChain = useCallback(async () => {
        if (!activeAddress) return;
        setIsLoadingBatches(true);
        setAllBatches([]);
        try {
            console.log("RPC: getBatchesByContributor");
            incrementRpcCount();
            const batchIds = await readContract({ contract, method: "function getBatchesByContributor(address) view returns (uint256[])", params: [activeAddress] }) as bigint[];
            if (batchIds.length === 0) { setAllBatches([]); setIsLoadingBatches(false); return; }
            
            const batchPromises = batchIds.map(id => {
                console.log(`RPC: getBatchInfo for ID ${id}`);
                incrementRpcCount();
                return readContract({ contract, method: "function getBatchInfo(uint256) view returns (uint256,address,string,string,string,string,string,string,bool)", params: [id] });
            });
            const batchesData = await Promise.all(batchPromises);
            
            const formattedBatches = batchesData.map((data, index): BatchData => ({
                id: batchIds[index].toString(), batchId: data[0], name: data[2], description: data[3], date: data[4], location: data[5], isClosed: data[8]
            }));
            
            setAllBatches(formattedBatches.reverse());
        } catch (error) {
            console.error("Errore durante il caricamento on-chain:", error);
            setTxResult({ status: 'error', message: 'Errore nel leggere i dati dalla blockchain.' });
        } finally {
            setIsLoadingBatches(false);
        }
    }, [activeAddress, incrementRpcCount]);

    useEffect(() => {
        if (prevAccountRef.current && !activeAddress) { window.location.href = "/"; } 
        else if (activeAddress && prevAccountRef.current !== activeAddress) {
            resetRpcCount();
            refetchContributorInfo();
        }
        prevAccountRef.current = activeAddress;
    }, [activeAddress, refetchContributorInfo, resetRpcCount]);

    useEffect(() => {
        if (contributorData && contributorData[2]) {
            fetchAllBatchesOnChain();
        }
    }, [contributorData, fetchAllBatchesOnChain]);
    ...
    const handleInitializeBatch = async () => {
        ...
        const transaction = prepareContractCall({ contract, method: "function initializeBatch(string,string,string,string,string)", params: [formData.name, formData.description, formData.date, formData.location, "N/A"] });
        sendAndConfirmTransaction(transaction, {
            onSuccess: async () => {
                setLoadingMessage("Transazione confermata! Aggiorno la lista...");
                setTxResult({ status: "success", message: "Iscrizione creata con successo!" });
                await fetchAllBatchesOnChain();
            ...
            onError: (error) => {
                ...
                const reason = (error as any).cause?.reason || (error as any).reason || "Nessun motivo specifico trovato.";
                setTxResult({ status: "error", message: `La transazione è fallita.\n\nMotivo: ${reason}` });
                setLoadingMessage("");
            },
        });
    };

    const openModal = () => { setModal("init"); setFormData(getInitialFormData()); setCurrentStep(1); setTxResult(null); };
    ...
    if (!activeAddress) {
        return ( <div className="login-container"> <AziendaPageStyles /> <ConnectButton client={client} chain={polygon} accountAbstraction={{ chain: polygon, sponsorGas: true }} connectModal={{ size: "wide", wallets: [inAppWallet()] }} connectButton={{ label: "Connettiti / Log In", style: { fontSize: "1.2rem", padding: "1rem 2rem" } }} /> </div> );
    }
    ...
    return (
        <div className="app-container-full">
            ...
            <main className="main-content-full">
                {isStatusLoading && <p style={{ textAlign: "center", marginTop: "4rem" }}>Verifica stato account...</p>}
                {isError && <p style={{ textAlign: "center", marginTop: "4rem", color: "red" }}>Errore nel recuperare i dati dell'account. Riprova.</p>}
                {contributorData && !contributorData[2] && <RegistrationForm />}
                {contributorData && contributorData[2] && (
                    <>
                        <DashboardHeader contributorInfo={contributorData} onNewInscriptionClick={openModal} onRefreshClick={fetchAllBatchesOnChain} isRefreshDisabled={isLoadingBatches} refreshTooltip={isLoadingBatches ? "Caricamento..." : "Ricarica dati da On-Chain"} />
                        {isLoadingBatches ? <p style={{ textAlign: "center", marginTop: "2rem" }}>Caricamento iscrizioni da blockchain...</p> : <BatchTable batches={filteredBatches} nameFilter={nameFilter} setNameFilter={setNameFilter} locationFilter={locationFilter} setLocationFilter={setLocationFilter} statusFilter={statusFilter} setStatusFilter={setStatusFilter} />}
                    </>
                )}
            </main>
            ...
            {modal === 'init' && (
                 ...
                 <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                     ...
                     <div className="modal-body" style={{ minHeight: "350px" }}>
                         {currentStep === 1 && ( <div> <div className="form-group"> <label>Nome Iscrizione <span style={{ color: "red", fontWeight: "bold" }}>* Obbligatorio</span></label> <input type="text" name="name" value={formData.name} onChange={handleModalInputChange} className="form-input" maxLength={100} /> <small className="char-counter">{formData.name.length} / 100</small> </div> <div style={{ backgroundColor: "#343a40", border: "1px solid #495057", borderRadius: "8px", padding: "16px", marginTop: "16px", fontSize: "0.9rem", color: "#f8f9fa" }}> <p> <strong>ℹ️ Come scegliere il Nome Iscrizione</strong> </p> <p> Il Nome Iscrizione è un'etichetta descrittiva che ti aiuta a identificare in modo chiaro ciò che stai registrando on-chain. Ad esempio: </p> <ul style={{ textAlign: "left", paddingLeft: "20px" }}> <li> Il nome di un prodotto o varietà:{" "} <em>Pomodori San Marzano 2025</em> </li> <li> Il numero di lotto: <em>Lotto LT1025 – Olio EVO 3L</em> </li> <li> Il nome di un contratto:{" "} <em>Contratto fornitura COOP – Aprile 2025</em> </li> <li> Una certificazione o audit:{" "} <em>Certificazione Bio ICEA 2025</em> </li> <li> Un riferimento amministrativo:{" "} <em>Ordine n.778 – Cliente NordItalia</em> </li> </ul> <p style={{ marginTop: "1rem" }}> <strong>📌 Consiglio:</strong> scegli un nome breve ma significativo, che ti aiuti a ritrovare facilmente l’iscrizione anche dopo mesi o anni. </p> </div> </div> )}
                         {currentStep === 2 && ( <div> <div className="form-group"> <label>Descrizione <span style={{ color: "#6c757d" }}>Non obbligatorio</span></label> <textarea name="description" value={formData.description} onChange={handleModalInputChange} className="form-input" rows={4} maxLength={500}></textarea> <small className="char-counter">{formData.description.length} / 500</small> </div> <div style={{ backgroundColor: "#343a40", border: "1px solid #495057", borderRadius: "8px", padding: "16px", marginTop: "16px", fontSize: "0.9rem", color: "#f8f9fa" }}> <p> Inserisci una descrizione del prodotto, lotto, contratto o altro elemento principale. Fornisci tutte le informazioni essenziali per identificarlo chiaramente nella filiera o nel contesto dell’iscrizione. </p> </div> </div> )}
                         {currentStep === 3 && ( <div> <div className="form-group"> <label>Luogo <span style={{ color: "#6c757d" }}>Non obbligatorio</span></label> <input type="text" name="location" value={formData.location} onChange={handleModalInputChange} className="form-input" maxLength={100} /> <small className="char-counter">{formData.location.length} / 100</small> </div> <div style={{ backgroundColor: "#343a40", border: "1px solid #495057", borderRadius: "8px", padding: "16px", marginTop: "16px", fontSize: "0.9rem", color: "#f8f9fa" }}> <p> Inserisci il luogo di origine o di produzione del prodotto o lotto. Può essere una città, una regione, un'azienda agricola o uno stabilimento specifico per identificare con precisione dove è stato realizzato. </p> </div> </div> )}
                         {currentStep === 4 && ( <div> <div className="form-group"> <label>Data <span style={{ color: "#6c757d" }}>Non obbligatorio</span></label> <input type="date" name="date" value={formData.date} onChange={handleModalInputChange} className="form-input" max={new Date().toISOString().split("T")[0]} /> </div> <div style={{ backgroundColor: "#343a40", border: "1px solid #495057", borderRadius: "8px", padding: "16px", marginTop: "16px", fontSize: "0.9rem", color: "#f8f9fa" }}> <p> Inserisci una data, puoi utilizzare il giorno attuale o una data precedente alla conferma di questa Iscrizione. </p> </div> </div> )}
                         {currentStep === 5 && ( <div> <div className="form-group"> <label>Immagine <span style={{ color: "#6c757d" }}>Non obbligatorio</span></label> <input type="file" name="image" onChange={handleFileChange} className="form-input" accept="image/png, image/jpeg, image/webp" /> <small style={{ marginTop: "4px" }}> Formati: PNG, JPG, WEBP. Max: 5 MB. </small> {selectedFile && ( <p className="file-name-preview"> File: {selectedFile.name} </p> )} </div> <div style={{ backgroundColor: "#343a40", border: "1px solid #495057", borderRadius: "8px", padding: "16px", marginTop: "16px", fontSize: "0.9rem", color: "#f8f9fa" }}> <p> Carica un’immagine rappresentativa del prodotto, lotto, contratto, etc. Rispetta i formati e i limiti di peso. </p> <p style={{ marginTop: "10px" }}> <strong>Consiglio:</strong> Per una visualizzazione ottimale, usa un'immagine quadrata (formato 1:1). </p> </div> </div> )}
                     ...
                     <div className="modal-footer" style={{ justifyContent: "space-between" }}>
                         <div> {currentStep > 1 && (<button onClick={handlePrevStep} className="web3-button secondary" disabled={isPending || !!loadingMessage}>Indietro</button>)} </div>
                         <div> <button onClick={handleCloseModal} className="web3-button secondary" disabled={isPending || !!loadingMessage}>Chiudi</button> {currentStep < 6 && (<button onClick={handleNextStep} className="web3-button">Avanti</button>)} {currentStep === 6 && (<button onClick={handleInitializeBatch} disabled={isPending || !!loadingMessage} className="web3-button">{ (isPending || !!loadingMessage) ? "Conferma..." : "Conferma e Registra"}</button>)} </div>
                     </div>
                 </div>
             </div>
            )}
            
            {(isPending || loadingMessage) && <TransactionStatusModal status={"loading"} message={loadingMessage || "In attesa di conferma..."} onClose={() => {}} />}
